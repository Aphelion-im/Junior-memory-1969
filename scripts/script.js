...
window.addEventListener('load', () => {
  ...
  const numberOfCardsSlider = document.querySelector('.number-of-cards-slider');
  ...
  */
  function createBoard() {
    for (let i = 0; i < numberOfCardsChosen; i++) {
      // let i = 0; i < cardsArray.length; i++
  ...
  function turnCard() {
    if (!busy) {
      ...
      flips++;
      const dataId =
        this.lastElementChild.lastElementChild.getAttribute('data-id');
      const dataCard =
  ...
  function checkMatch() {
    ...
    if (choiceId1 === choiceId2) {
      ...
      cards[choiceId1].removeAttribute('title');
      cards[choiceId2].removeAttribute('title');
    }
